...
public class Node extends GameBoard {
	 ...
	 */
	public Node(ArrayList<Integer> configuration, int g) {
		super(configuration);
		this.g = g;
		this.h = this.linearConflict(); // Heuristic score
	 ...
	 */
	public void addChild(ArrayList<Integer> configuration, int gScore) {
		Node child = new Node(configuration, gScore);
	...
	public int manhattan() {
		...
		for (int i = 0; i <= this.getHeight() * (this.getHeight() - 1); i += this.getHeight()) {
			for (int j = 0; j < this.getHeight(); j++) {
				int value = this.getBoard().get(i + j);
				if (value != 0) {
	...
	public int linearConflict() {
		...
		for (int ri = 0; ri < this.getHeight(); ri++) {
			...
			ArrayList<Integer> newConfigurationRow = new ArrayList<>(this.getBoard());
			...
			ArrayList<Integer> newConfigurationCol = new ArrayList<>(this.getBoard());
			int maxColConflictor = 0;
		...
		return 2 * linearConflicts + this.manhattan();
	}
	...
	/**
	 * Method to retrieve the number of linear conflicts that a given tile has in its row
	 * 
	...
	/**
	 * Method to retrieve the number of linear conflicts that a given tile has in its column
	 * 
	 * @param index of the tile
	 * @param configuration 
	 */
