...
public class PuzzlePanel extends JPanel {
	...
	/**
	 * Print a start message and instructions for the game
	 * 
	 * @param g the grid to be used
	 */
	private void drawMessage(Graphics2D window) {
		if (solved) {
			window.setFont(getFont().deriveFont(Font.BOLD, 18));
			window.setColor(Color.GRAY);
			String moves = "Number of moves made:";
			window.drawString(moves, (getWidth() - window.getFontMetrics().stringWidth(moves)) / 2, getHeight() - margin);
		}
	}
